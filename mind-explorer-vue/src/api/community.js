// 社区交流前端 API —— 经 Supabase Edge Function content 中转（service_role 读写）
// 0012：从 /.netlify/functions/community 迁移；action 命名空间化（post.*）；
// 不再直连/不再暴露额外密钥，统一的错误/危机检测在服务端完成。
...
export const communityApi = {
  // 帖子列表（时间倒序，公开）
  async getPosts() {
    const data = await call('post.list')
    return data.posts || []
  },
  ...
  async addPost(titleOrPayload, content, user) {
    ...
    const data = await call('post.add', {
      title: payload.title,
  ...
  // 获取单个帖子
  async getPost(id) {
    const data = await call('post.get', { id })
    return data.post || null
  },

  // 获取帖子评论
  async getComments(postId) {
    const data = await call('post.comments', { postId })
    return data.comments || []
  },

  // 发表评论（返回新建的评论行）
  async addComment(postId, content, user) {
    const data = await call('post.comment', { postId, content, user: user || {} })
    return data.comment || null
  },

  // 获取单帖认同列表（[{ user_identifier }]）
  async getLikes(postId) {
    const data = await call('post.likes', { postId })
    return data.likes || []
  },

  // 批量认同数：传入 post id 数组，返回 { [postId]: count }
  async getLikesBatch(ids) {
    const data = await call('post.likesBatch', { ids: ids || [] })
    return data.map || {}
  },

  // 当前用户在给定帖子里的已认同集合：返回 [post_id,...]
  async myLikes(ids, userIdentifier) {
    const data = await call('post.myLikes', { ids: ids || [], user_identifier: userIdentifier })
    return data.liked || []
  },
  ...
  // 懒触发：前端「每日首次打开」时若今日无自动推送，调用 content 的 sunny.push 即时拉取
  // 返回 { ok, pushed, skipped, count } 或抛出错误
  async triggerSunnyPush(force = false) {
    return call('sunny.push', force ? { force: true } : {})
  },
}
